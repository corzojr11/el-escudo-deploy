...
export function PlanSemanalClient({ missions, habits, shifts, personalEntries, loadErrors }: { missions: Mission[]; habits: Habit[]; shifts: Shift[]; personalEntries: PersonalEntry[]; loadErrors: number }) {
  ...
  function createChallenge() {
    ...
    run(async () => { await createPersonalEntry({ kind: "discipline", title: challengeTitle.trim(), data: { tracker_type: "adaptive_challenge", duration: challengeDays, start_date: today, completed_dates: [] } }); setChallengeTitle(""); setMessage("Desafio creado. Si fallas un dia, vuelves al siguiente sin reiniciar todo."); });
  }
  ...
  function saveRoutine() {
    ...
    run(async () => {
      if (morningRoutine) await updatePersonalEntry(morningRoutine.id, { title: "Rutina matutina", content: "Fisico, mental y espiritual.", data });
      else await createPersonalEntry({ kind: "discipline", title: "Rutina matutina", content: "Fisico, mental y espiritual.", data });
  ...
  return <div className="mx-auto flex w-full max-w-7xl flex-col gap-5 pb-8">
    <Card className="border-[#2A2A3C] bg-[#17171A]"><CardHeader><p className="hud-label text-[#bcaeff]">SISTEMA DE SEGUIMIENTO</p><CardTitle className="text-3xl text-white">Tu semana, tus retos y tu energia</CardTitle><CardDescription>Un tablero vivo: eliges acciones, mides constancia y retomas el rumbo sin castigos cuando una semana se complica.</CardDescription></CardHeader></Card>
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <Card className="border-[#2A2A3C] bg-[#17171A]"><CardHeader><p className="hud-label text-[#bcaeff]">RUTINA MATUTINA</p><CardTitle className="flex items-center gap-2 text-white"><Sparkles className="h-5 w-5 text-[#FFD700]" /> Fisico, mental y espiritual</CardTitle><CardDescription>Empieza pequeno: una accion por area vale mas que una rutina perfecta abandonada.</CardDescription></CardHeader><CardContent className="space-y-3">{morningRoutine ? <>{(["physical", "mental", "spiritual"] as const).map((area) => { const label = area === "physical" ? "Fisico" : area === "mental" ? "Mental" : "Espiritual"; const activity = typeof morningRoutine.data[area] === "string" ? morningRoutine.data[area] : "Sin definir"; return <button key={area} disabled={isPending} onClick={() => toggleArea(area)} className="flex w-full items-center gap-3 border border-[#2A2A3C] p-3 text-left"><span className={checkedAreas.includes(area) ? "text-[#7C5DFF]" : "text-muted-foreground"}>{checkedAreas.includes(area) ? <Check className="h-5 w-5" /> : <Circle className="h-5 w-5" />}</span><span><b className="block text-sm text-white">{label}</b><span className="text-xs text-muted-foreground">{activity}</span></span></button>; })}<p className="text-xs text-muted-foreground">{checkedAreas.length}/3 areas cumplidas hoy.</p></> : <><Input value={routineDraft.physical} onChange={(event) => setRoutineDraft({ ...routineDraft, physical: event.target.value })} placeholder="Fisico: Ej. 15 min de movilidad" className="border-[#2A2A3C] bg-[#0C0C0E] text-white" /><Input value={routineDraft.mental} onChange={(event) => setRoutineDraft({ ...routineDraft, mental: event.target.value })} placeholder="Mental: Ej. leer 10 paginas" className="border-[#2A2A3C] bg-[#0C0C0E] text-white" /><Input value={routineDraft.spiritual} onChange={(event) => setRoutineDraft({ ...routineDraft, spiritual: event.target.value })} placeholder="Espiritual: Ej. oracion o gratitud" className="border-[#2A2A3C] bg-[#0C0C0E] text-white" /><Button disabled={isPending} onClick={saveRoutine} className="w-full bg-[#7C5DFF]">Guardar mi rutina</Button></>}</CardContent></Card>
    </div>
    <Card className="border-[#2A2A3C] bg-[#17171A]"><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="hud-label text-[#bcaeff]">PLAN SEMANAL</p><CardTitle className="text-white">Acciones, turnos y habitos</CardTitle></div><Link className="flex items-center gap-2 text-sm text-[#FFD700] hover:underline" href="/finanzas"><Wallet className="h-4 w-4" /> Abrir protocolo financiero</Link></div></CardHeader><CardContent><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-7">{days.map((day) => { const dayMissions = missions.filter((mission) => missionDate(mission) === day.date); const shift = shifts.find((item) => item.day.toLowerCase().normalize("NFD").replace(/[^a-z]/g, "") === day.key); const habitsDone = habits.filter((habit) => habit.completed_dates?.some((date) => date.slice(0, 10) === day.date)).length; return <div key={day.date} className={`min-h-64 border p-3 ${day.date === today ? "border-[#7C5DFF]" : "border-[#2A2A3C]"}`}><div className="mb-3 flex justify-between"><b className="text-sm text-white">{day.label}</b><span className="font-mono text-xs text-[#bcaeff]">{day.date.slice(8)}</span></div><p className="mb-2 text-xs text-muted-foreground">{shift ? `Turno ${shift.start}-${shift.end}` : "Sin turno"} · Habitos {habitsDone}/{habits.length}</p><div className="space-y-2">{dayMissions.map((mission) => <button key={mission.id} disabled={isPending} onClick={() => toggleMission(mission)} className="flex w-full gap-2 text-left text-xs"><span className={mission.status === "completed" ? "text-[#7C5DFF]" : "text-muted-foreground"}>{mission.status === "completed" ? <Check className="h-4 w-4" /> : <Circle className="h-4 w-4" />}</span><span className={mission.status === "completed" ? "line-through text-muted-foreground" : "text-white"}>{mission.name ?? mission.title}</span></button>)}</div>{draftDay === day.date ? <div className="mt-3 space-y-2"><Input autoFocus value={draftName} onChange={(event) => setDraftName(event.target.value)} placeholder="Accion concreta" className="h-8 border-[#2A2A3C] bg-[#0C0C0E] text-xs text-white" /><Button size="sm" disabled={isPending || !draftName.trim()} onClick={() => createForDay(day.date)} className="w-full bg-[#7C5DFF]">Guardar</Button></div> : <button onClick={() => { setDraftDay(day.date); setDraftName(""); }} className="mt-3 flex items-center gap-1 text-xs text-[#d5ccff]"><Plus className="h-3.5 w-3.5" /> Agregar accion</button>}</div>; })}</div></CardContent></Card>
