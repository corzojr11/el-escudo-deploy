...
DAYS.forEach((name, i) => {
  DAY_NORMALIZE_MAP[name.toLowerCase().normalize("NFD").replace(/[^a-z]/g, "")] = DAY_KEYS[i];
  DAY_NORMALIZE_MAP[name.toLowerCase()] = DAY_KEYS[i];
  DAY_NORMALIZE_MAP[DAY_KEYS[i]] = DAY_KEYS[i];
});

function normalizeDay(raw: string): string {
  const cleaned = (raw || "").trim();
  const lower = cleaned.toLowerCase();
  const stripped = lower.normalize("NFD").replace(/[^a-z]/g, "");
  return DAY_NORMALIZE_MAP[stripped] || DAY_NORMALIZE_MAP[lower] || stripped;
}
